...
import yaml
from course import Course,ExploreCourse
...
class Profile:
    def __init__(self, explore_course: ExploreCourse, prefsPath: str):
        """
        ...
        with open(prefsPath, "r") as profile_file:
            self.profile = yaml.load(profile_file)

        self.minUnits = self.profile.get("minUnits")
        self.maxUnits = self.profile.get("maxUnits")

        for course in self.profile.get("taken"):
            self.taken.add(course)

        for quarter in self.profile.get("register"):
            self.quarters.append(quarter)

        for request in self.profile.get("request"):
            cids = [request.get("course")]
            quarters = []
            prereqs = []
            weight = 1
                ...
                for prereq_cid in course.prereqs:
                    if prereq_cid in self.taken:
                        ...
                        if prereq_cid not in req.prereqs:
                            ...
                            print(
                                f"INFO: Additional prereqs inferred: {cid} after {prereq_cid}"
                            )
                    else:
                        print(
                            f"WARNING: missing prerequisite of {cid} -- \
